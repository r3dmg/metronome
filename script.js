window.addEventListener('DOMContentLoaded',()=>{
    ...
    function stop(){ if(!isRunning) return; isRunning=false; if(timerID){clearTimeout(timerID); timerID=null;} startBtn.disabled=false; stopBtn.disabled=true; currentBeat=0; currentBar=1; runToken++; clearVisuals(); stopUITimer(); nextStepTimer.textContent='—:—'; nextStepTimer.style.visibility='hidden'; if(totalTimerID){ clearInterval(totalTimerID); totalTimerID=null; } audioCtx = null; samples = {kick:null,snare:null,hhc:null,hho:null,crash:null,tom:null}; }
